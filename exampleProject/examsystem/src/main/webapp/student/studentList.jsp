...
function deleteStudent(studentId) {
	if(confirm("确认删除这条记录吗?")){
		$.post("student!deleteStudent",{hidden_id:studentId},
				function(result) {
				//	alert(result);
		...
		<form action="${pageContext.request.contextPath}/student!list" method="post">
			<table align="center">
				<tr>
					...
					<td><input type="text" id="s_id" name="id" value="${student.id }"/></td>
					<td>&nbsp;</td>
					<td><label>姓名：</label></td>
					<td><input type="text" id="s_name" name="name" value="${student.name }"/></td>
					<td>&nbsp;</td>
	...
	<div class="data_content">
		<table class="table table-bordered table-hover">
			...
			<tr>
				...
				<td><button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='student!preSave?hidden_id=${student.id}'">修改</button>&nbsp;&nbsp;<button class="btn btn-mini btn-danger" type="button" onclick="deleteStudent('${student.id}')">删除</button></td>
			</tr>
			</c:forEach>
		</table>
	</div>
