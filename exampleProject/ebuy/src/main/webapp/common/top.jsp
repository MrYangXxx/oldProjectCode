...
<div class="help">
	...
	<form action="product_.action" method="post">
		<input type="text" id="txtSearch" name="product.name"
			onkeyup="" autocomplete="off" value="${product.name }" /> <input
...
<div class="navbar">
	<ul class="clearfix">
		...
		<c:forEach var="bigType" items="${bigTypeList }">
			<li>
				<a href="product_.action?product.bigType.id=${bigType.id }">${bigType.name }</a>
			</li>
		</c:forEach>
	</ul>
</div>
